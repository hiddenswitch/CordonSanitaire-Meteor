 ...
 * @param position {{x: number, y: number}} The start position to route from.
...
SanitairePatientZero.estimatePositionFromPath = function (speed, path, startTime, options) {
    ...
    var tilesTraveled = Math.round((options.time - startTime) * (speed / options.tileSize));
    return path[Math.max(Math.min(path.length - 1, tilesTraveled), 0)];
};

SanitairePatientZero.think = function (gameId) {
    Meteor.setInterval(function (timerId) {
        // Set the timer ID here
