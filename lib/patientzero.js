...
SanitairePatientZero.computeNewRoute = function (tiles, position, options) {
    ...
    if (!crosswalks) {
        ...
        var crosswalkTiles = _.object(SanitaireMaps.CROSSWALK_TILES, []);
...
SanitairePatientZero.estimatePositionFromPath = function (speed, path, startTime, options) {
    ...
    var time = Meteor.isClient ? TimeSync.serverTime(new Date()) : (new Date()).getTime();
    var tileSize = SanitaireMaps.TILE_SIZE;
    var tilesTraveled = Math.floor(((time - startTime) / 1000) * (speed / tileSize));
