...
Sanitaire._getBarricadePositions = function (game) {
    var intersectionIds = _.compact(_.map(game.barriers, function (v, k) {
        var now = new Date();
        if (v.barrierExistsTime <= now && v.barrierStopsExistingTime > now) {
...
Sanitaire.addConstructionMessageToLog = function (gameId, playerId, intersectionId, messageType, time) {
    ...
    // Interpret the log with the following hierarchy:
    // {intersectionId: {playerId: [log entries sorted by time]}}
    // var todo = _.mapObject(_(game.barriersLog).groupBy('intersectionId'), function(logEntries, intersectionId) {
    //     return _.groupBy(_.sortBy(logEntries, 'time'), 'playerId');
    // });
