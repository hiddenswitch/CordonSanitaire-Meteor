 ...
 * @type {number}
 */
Sanitaire.MAX_PLAYERS = Meteor.settings && Meteor.settings.maxPlayers || 4;
/**
 * How long should a game last in seconds?
 * @type {number}
 */
Sanitaire.DURATION_SECONDS = Meteor.settings && Meteor.settings.durationSeconds || 45;
/**
 * How long should the countdown last in seconds?
 * @type {number}
 */
Sanitaire.COUNTDOWN_SECONDS = Meteor.settings && Meteor.settings.countdownSeconds || 10;
/**
 * How long should it take to build/complete a single tile barricade in milliseconds?
 * @type {number}
 */
Sanitaire.BARRICADE_CONSTRUCTION_MILLIS = Meteor.settings && Meteor.settings.constructionMillis || 1000;
/**
...
Sanitaire.DEFAULT_MAP = 'London.csv';   //'Simple_40.csv';
 ...
 * @private
 */
Sanitaire._getQuarantinePositions = function (game) {
    return _.compact(_.map(game, function (v, k) {
...
Sanitaire._changePatientZeroPath = function (game, map, quarantineTiles) {
    ...
    quarantineTiles = quarantineTiles || Sanitaire._getQuarantinePositions(game);
...
Sanitaire.addConstructionMessageToLog = function(gameId, playerId, intersectionId, messageType, time) {
    ...
    //Todo: interpret construction log and calculate where and when barriers will be complete
    // this is barricade states interpreted

    //Todo: update barriers array to contain the time when a given barrier will appear
    //Todo: update barrier or create barrier if it doesn't exist
    // save the barricade state to the database (game document)
    //var forever = new Date(9999,12,31); // basically infinity
    //Games.update(game._id, {
    //    $addToSet: {
    //        barriers: {
    //            intersectionId: intersectionId,
    //            barrierExistsTime: forever,
    //            barrierStopsExistingTime: forever
    //        }
    //    }
    //}
