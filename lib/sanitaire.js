...
Sanitaire.addConstructionMessageToLog = function (gameId, playerId, intersectionId, messageType, time) {
    ...
    var barriers = _.map(_.groupBy(game.barriersLog, 'intersectionId'), function (logEntries, intersectionId) {
        ...
        var line = _.reduce(logEntries, function (line, logEntry) {
            ...
            // y = m(x - x_1) + y_1
            var logX = logEntry.getTime();
            var logY = logEntry.m * (logX - logEntry.x_1) + logEntry.y_1;
            var m = logEntry.m;
            // Compute new slope
        ...
        // Specially handle clamped values
        ...
        return {
            intersectionId: intersectionId,
            barrierExistsTime: new Date(barrierExistsTime),
            barrierStopsExistingTime: new Date(barrierStopsExistingTime)
        };
    });
