...
/**
 * Game states.
 * @type {{LOBBY: string, COUNTDOWN: string, IN_PROGRESS: string, ENDED: string, EXPIRED: string}}
 */
Sanitaire.gameStates = {
    ...
    ENDED: 'ended',
    /**
     * Expired means this game did not get enough players to exist
     */
    EXPIRED: 'expired'
};
 ...
 * @type {any|*|number}
 */
Sanitaire.BARRICADE_BUILD_DURATION_SECONDS = Meteor.settings && Meteor.settings.public && Meteor.settings.public.buildTimeSeconds || 12;
/**
 * How long it will take to demolish a barricade alone
 * @type {any|*|number}
 */
Sanitaire.BARRICADE_DEMO_DURATION_SECONDS = Meteor.settings && Meteor.settings.public && Meteor.settings.public.demolishTimeSeconds || 3;
/**
 * How long it will take to recover from being stunned
 * @type {any|*|number}
 */
Sanitaire.STUN_DURATION_SECONDS = Meteor.settings && Meteor.settings.public && Meteor.settings.public.stunTimeSeconds || 60;
/**
 * What is the max number of players per game?
 * @type {number}
 */
Sanitaire.MAX_PLAYERS = Meteor.settings && Meteor.settings.public && Meteor.settings.public.maxPlayers || 10;
/**
 * How long should a game last in seconds?
 * @type {number}
 */
Sanitaire.DURATION_SECONDS = Meteor.settings && Meteor.settings.public && Meteor.settings.public.durationSeconds || 60;
/**
 * How long should the countdown last in seconds?
 * @type {number}
 */
Sanitaire.COUNTDOWN_SECONDS = Meteor.settings && Meteor.settings.public && Meteor.settings.public.countdownSeconds || 5;
/**
 * How long should users wait in an incomplete lobby without someone joining
 * @type {number}
 */
Sanitaire.MAX_LOBBY_TIME_SECONDS = Meteor.settings && Meteor.settings.public && Meteor.settings.public.maximumLobbyWaitSeconds || 60;
/**
...
/**
 * path to the csv for a map
 * @type {String}
 */
Sanitaire.DEFAULT_MAP = Meteor.settings && Meteor.settings.public && Meteor.settings.public.map || "Simple_Single_02.csv";
/**
 * Should we have the same color suits or be unique?
 * @type {boolean}
 */
Sanitaire.DEFAULT_ZOOM = Meteor.settings && Meteor.settings.public && Meteor.settings.public.zoom || 1;
/**
 * Should we have the same color suits or be unique?
 * @type {boolean}
 */
...
/**
 * Only show the let's play for the first 5 minutes of the hour
 * @type {[number]} - array of valid minute times or no restriction string value
 */
Sanitaire.TIME_RESTRICTED_ENTRY = Meteor.settings && Meteor.settings.public && Meteor.settings.public.timeRestrictedEntry || "NO_RESTRICTION";
...
/**
 * Expires a lobby due to not enough responders
 * @param gameId
 * @private
 */
Sanitaire._expireGame = function (gameId) {
    // Stop computing new patient zero behaviors
    // probably shouldn't be a p0 since the game never starts
    SanitairePatientZero.stopThink(gameId);
    Games.update(gameId, {$set: {
        joinedPlayerIds: [],
        joinedUserIds: [],
        state: Sanitaire.gameStates.EXPIRED}});
};
...
Sanitaire._startCountdownForGame = function (gameId) {
    ...
    var isExpired = (game.state === Sanitaire.gameStates.EXPIRED);
...
Sanitaire._addPlayerToGame = function (gameId, userId) {
    ...
    var playerId = Players.insert({
        ...
        // Set the position at the start of countdown.
        position: {x: 0, y: 0},
        velocity: {x: 0, y: 0}
    });
...
Sanitaire._changePatientZeroPath = function (game, map, quarantineTiles) {
    ...
    _.each(quarantineTiles, function (tile) {
        tiles[tile.y][tile.x] = SanitaireMaps.QUARANTINE_TILES[0];
    });
