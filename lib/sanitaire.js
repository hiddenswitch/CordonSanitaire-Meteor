...
/**
 * Updates the player's health in the case of being touched by patientZero
 * @param playerId
 * @param health
 * @param touchedTime
 * @returns {boolean}
 */
Sanitaire.updatePlayerHealth = function (playerId, health, touchedTime) {
    check(playerId, String);
    check(health, Number);
    check(touchedTime, Number);

    return !!Players.update({_id: playerId},
        {
            $set: {health: {value: health, lastTouched: touchedTime}}
        });
};
