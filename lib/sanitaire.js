 ...
 * @type {any|*|number}
 */
Sanitaire.STUN_DURATION_SECONDS = Meteor.settings && Meteor.settings.public && Meteor.settings.public.stunTimeSeconds || 5;
/**
...
/**
 * Wall tiles...
 * Todo: move this to SanitaireMap...
 * @type {number[]}
 */
...
Sanitaire._createGame = function () {
    ...
    var game = {
        ...
        //})
        gameStats:{
            //TODO: Does p0 always start off as loose?
            patientZeroStatus: SanitairePatientZero.statuses.LOOSE,
            numberOfBarricadesBuilt: 0,
            numberOfPlayersInjured: 0,
            numberOfQuarantines:0
        }
    };
...
Sanitaire._endGame = function (gameId) {
    ...
    Sanitaire.updateGameStatsNumberOfPlayersInjured(gameId);
};
...
Sanitaire.addConstructionMessageToLog = function (gameId, playerId, intersectionId, messageType, time) {
    ...
    _.each(barriers,function(barrier){
        if (barrier.state === 3){ // barricade built
            numBarriers += 1;
        }
    });
    ...
    Games.update(gameId, {
        $set: {
            barriers: barriers,
            "gameStats.numberOfBarricadesBuilt": numBarriers
        },
        $addToSet: {
...
/**
 * Updates patient zero status (isolated/loose)
 * @param gameId {Number}
 * @param patientZeroStatus {String}
 */
Sanitaire.updateGameStatsPatientZeroStatus = function(gameId, patientZeroStatus){
    var game = Games.findOne(gameId);
    var patientZeroOldStatus = game.gameStats.patientZeroStatus;
    if (!(patientZeroStatus === patientZeroOldStatus)){
        Games.update(gameId, {
            $set: {
                "gameStats.patientZeroStatus": patientZeroStatus
            }
        });
    }
};

Sanitaire.updateGameStatsNumberOfPlayersInjured = function(gameId){
    var game = Games.findOne(gameId);
    var playerIds = game.joinedPlayerIds;
    var numInjured = 0;
    for (var i=0; i<playerIds.length;i++){
        var playerId = playerIds[i];
        if (Players.findOne(playerId).injuredTimeLog.length>0){
            numInjured++;
        }
    }
    Games.update(gameId, {
        $set: {
            "gameStats.numberOfPlayersInjured": numInjured
        }
    });
};
 ...
 * @returns {boolean}
 */
Sanitaire.updatePlayerTouchedByPatientZero = function(playerId, injuredTime){
    return !!Players.update({_id: playerId},
        {
            $addToSet: {injuredTimeLog: injuredTime}
        });
};


Sanitaire.updateNumberOfQuarantines = function(gameId, numQuarantines){
    var game = Games.findOne(gameId);
    if (!(game.state === Sanitaire.gameStates.ENDED)){
        Games.update(gameId, {
            $set: {
                "gameStats.numberOfQuarantines": numQuarantines
            }
        });
    }
}